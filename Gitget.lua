...
local preset = {
    ...
    done = function() if not silent then print("Done") end end
}
...
if #args < 2 then
    print("Usage:\n" .. ((shell and shell.getRunningProgram()) or "gitget") ..
              " <user> <repo> [branch/tree] [path]")
    error()
end

local function save(data, file)
    local file = shell.resolve(file:gsub("%%20", " "))
    if not (fs.exists(string.sub(file, 1, #file - #fs.getName(file))) and
        fs.isDir(string.sub(file, 1, #file - #fs.getName(file)))) then
...
local function download(url, file) save(http.get(url).readAll(), file) end
...
preset.start()
local data = json.decode(http.get("https://api.github.com/repos/" .. args[1] ..
                                      "/" .. args[2] .. "/git/trees/" .. args[3] ..
                                      "?recursive=1").readAll())
...
if data.message and data.message == "Not found" then
    ...
    for k, v in pairs(data.tree) do
        ...
        if v.type == "blob" then
            ...
            local url =
                "https://raw.github.com/" .. args[1] .. "/" .. args[2] .. "/" ..
                    args[3] .. "/" .. v.path, fs.combine(args[4], v.path)
            if async then
                http.request(url)
                ...
                if not silent then
                    print(fs.combine(args[4], v.path))
                end
            end
        end
    end
